<?php
    ...
    if(isset($_GET['cari'])){
        $getCari = $_GET["cari"];
        $queryToko = "SELECT kd_produk,tanggal,barang.merk_spt,jenis_spt,ukur_spt,jumlah,barang.kd_spt
                      FROM barang
                      JOIN toko ON barang.kd_spt = toko.kd_spt WHERE kd_produk LIKE '%$getCari%'";
    }else{
        $queryToko = "SELECT kd_produk,tanggal,barang.merk_spt,jenis_spt,ukur_spt,jumlah,barang.kd_spt
                      FROM barang JOIN toko ON barang.kd_spt = toko.kd_spt";
    }
    $resultToko = mysqli_query($conn,$queryToko);
    $countToko = mysqli_num_rows($resultToko);
?>
<html lang="en">
...
<head>
    ...
    <title>Toko Sepatu Super</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-GLhlTQ8iRABdZLl6O3oVMWSktQOp6b7In1Zl3/Jr59b6EGGoI1aFkw7cmDA6j6gD" crossorigin="anonymous">
    <link href="../css/styleToko.css" rel="stylesheet">
</head>
...
<body id="home">
    ...
    <section class="jumbotron text-center">
        ...
        <h2 class="display-3 fw-bold text-white">Toko Sepatu Super</h2>
        <p class="lead">Tempat Penjualan Sepatu paling terpercaya di seluruh indonesia</p>
    ...
    <section id="stok">
        <div class="container">
            <div class="row text-center mb-3">
                <div class="col">
                    <h2>Data Sepatu Di Toko</h2>
                </div>
            </div>
            <form action="tokoView.php#stok" method="POST">
                ...
                <div class="col">
                    <table class="table">
                        ...
                        <?php
                            $i = 1;
	                        if ($countToko > 0){
		                        while($dataToko=mysqli_fetch_array($resultToko)){
	                    ?>
                        <tr class="text-center">
                            <td class="value"><?php echo $i++; ?></td>
                            <td class="value"><?php echo "$dataToko[0]"; ?></td>
                            <td class="value"><?php echo "$dataToko[1]"; ?></td>
                            <td class="value"><?php echo "$dataToko[2]"; ?></td>
                            <td class="value"><?php echo "$dataToko[3]"; ?></td>
                            <td class="value"><?php echo "$dataToko[4]"; ?></td>
                            <td class="value"><?php echo "$dataToko[5]"; ?></td>
                            <td class="value">
                                <a class="text-dark text-decoration-none"
                                    href="tokoEdit.php?kd_produk=<?php echo"$dataToko[0]"; ?>&kd_spt=<?php echo"$dataToko[6]"; ?>">Edit
                                    | </a>
                                <a class="text-dark text-decoration-none"
                                    href="tokoDelete.php?kd_produk=<?php echo"$dataToko[0]"; ?>&kd_spt=<?php echo"$dataToko[6]"; ?>">Hapus</a>
                            </td>
                        </tr>
                        <?php
                        }
		                        ...
		                        echo"<tr>
				                    ...
				                    <div>BELUM ADA DATA TOKO</div></td>
			                        </tr>";
	                        }
                            echo"</table>";
                        ?>
            ...
            <div class="row text-center justify-content-center">
                <div class="col-md-4">
                    <a href="tokoAdd.php"><input name="add" type="submit" class="btn btn-dark mt-4"
                            value="Tambah Data Toko" /></a>
                </div>
            </div>
        </div>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
